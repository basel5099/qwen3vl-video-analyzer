...
def main():
    ...
    if user_prompt:
        ...
        planner_text, _pu = text_llm((
                ...
                "IMPORTANT: the segment analyst is a SMALL, much less capable "
                "vision model (8B). It follows short, simple, concrete, "
                "literal instructions well, but fails with abstract wording, "
                "multi-part logic, or clever phrasing. Design the instruction "
                "for THAT audience: short plain sentences, name the concrete "
                "visual things to look for and report, one clear task, no "
                "reasoning demands, no meta-language.\n"
        ...
        if output_schema:
            MERGE_PROMPT = (
                ...
                "Below are the sequential segment observations. They come "
                "from a small vision model: expect naming imprecision and "
                "duplicate mentions of the same entity across segments — "
                "reconcile and deduplicate carefully. All timestamps "
